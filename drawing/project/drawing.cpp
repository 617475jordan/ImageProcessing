...
Mat drawing::drawingContourMomentAndPointCentroid(Mat m_matCommonImg, Mat m_matMainImg)
{
	...
	//imshow("m_matMainImg", m_matMainImg);
	if (m_vecContours.size() == 0)
	{
		return m_matMainImg;
	}
	for (int i = 0; i < m_vecContours.size(); i++)
	{
		...
		int m_iCurrentArea = contourArea(m_vecContours[i]);
		if (m_iCurrentArea>100 /*&& m_iCurrentArea<m_dAreaThreshold*5*/)
		{
...
Mat drawing::drawingImage( Mat m_matMainImg)
{
	...
	{
		rectangle(drawing, m_vecRectangle[i], m_scalarTextColor);
	}
	imshow("d", drawing);*/
	//计算轮廓矩
	for (int i = 0; i < m_vecContours.size(); i++)
	...
	for (int i = 0; i < m_vecRectangle.size(); i++)
	{
		...
		if (matchShape.findMainRectContours(m_rect) == true)
		{
			...
			//cout << "m_iMaxSimilarityFlag:" << m_iMaxSimilarityFlag << endl;
